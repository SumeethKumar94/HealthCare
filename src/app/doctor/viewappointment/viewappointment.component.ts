...
this.doctorService.fetchStock(id).subscribe((res)=>this.MedMax=res,(error)=>console.log(error));
  ...
  addTestPrescription(TId: number) {
    ...
    {
     // console.log(this.TestDetails[i].TestId+' and ' +TId);
      if(this.TestDetails[i].TestId==+TId)
      { 
        flg=1;
    ...
    if(flg==1)
    this.toastr.error('Test Details Already Exists','Test Details');
    else
    this.TestDetails.push(this.testDetails);
  }
    //------Delete Test Details-------------
    deleteTest(testIdindex:number)
    {
    this.TestDetails.splice(testIdindex,1);
    }
    //--------------------------------
  TestSubmit() {
    for (let i = 0; i < this.TestDetails.length; i++) {
